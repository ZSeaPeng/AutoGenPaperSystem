...
export const getInitialState = () => dispatch => {
    return fetch('localhost:8111/AutoGenPaperSystem/api/subjectlist')
...
export const getSelect = url => dispatch => {
    return fetch(`localhost:8111/AutoGenPaperSystem/api${url}`)
        .then( response => response.json())
        .then( json =>
...
export const getQuestion = (url, query="?page=1") => dispatch => {
    return fetch(`localhost:8111/AutoGenPaperSystem/api${url}/question${query}`)
        .then( response => response.json())
        .then( json =>
